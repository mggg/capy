...
for (area_code, city_name, cluster), grp in df.groupby(['area_code', 'city_name', 'cluster'], sort=True):
    ...
    for ax, buf in zip(axes, SHOW_BUFFERS):
        ...
        ax.plot(data['year'], data['half_edge'], color='steelblue', marker='o', markersize=4, linewidth=1.5)
        axt.plot(data['year'], data['spread'], color='darkorange', marker='s', markersize=4,
                 linewidth=1.5, linestyle='--')

        ax.set_ylabel("Capy", color='steelblue', fontsize=8)
    ...
    figure_path = EXPERIMENT_DIR / "figures" / f"{city_name}_{cluster}_half_edge_spread_by_buffer.png"
