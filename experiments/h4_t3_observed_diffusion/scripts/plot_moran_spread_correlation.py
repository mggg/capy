...
ax.scatter(df['spread'], df['moran'], c=df['buffer_size'], cmap=cmap_buf, norm=norm_buf, s=25, alpha=0.7, linewidths=0)
...
ax.plot(x_b0, m_b0 * x_b0 + b_b0, color='tomato', linewidth=1.5, linestyle='--', label=f'Buffer = 0 only; Pearson corr = {buf0["moran"].corr(buf0["spread"]):.2f}')
