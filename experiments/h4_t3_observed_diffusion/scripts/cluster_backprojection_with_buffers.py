...
BUFFER_LIST = range(10) # buffer size
...
for CBSA in CBSA_CONFIG.keys():
    ...
    BLACK_SHARE_THRESHOLD = compute_rho(G_city_2020)
        ...
        gpkg_2020 = CLIPPED_GEO_DIR / "2020" / f"tracts_in_max_city_{CBSA}_2020_2020_vintage.gpkg"
        ...
        for year in YEARS:
            gpkg_path = CLIPPED_GEO_DIR / str(year) / f"tracts_in_max_city_{CBSA}_{year}_2020_vintage.gpkg"
            ...
            graph_file = DUAL_GRAPHS_DIR / str(year) / f"tracts_in_max_city_{CBSA}_{year}_2020_vintage_connected.json"
            with open(graph_file) as f:
