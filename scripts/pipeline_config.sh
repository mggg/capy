...
if [ "${STUDY_AREA_TYPE}" = "cbsa" ]; then
    ...
    if [ -z "${STUDY_AREA_SOURCE_FILE:-}" ]; then
        STUDY_AREA_SOURCE_FILE="$(find data/interim/study_area_sources -maxdepth 1 -name "${STUDY_AREA_SOURCE_PATTERN}" | sort | tail -n 1)"
    fi
...
STUDY_AREA_DEFINITION_GEOGRAPHIES="${STUDY_AREA_DEFINITION_GEOGRAPHIES:-data/interim/census_geographies/${STUDY_AREA_DEFINITION_GEOGRAPHY_YEAR}_${STUDY_AREA_DEFINITION_GEOGRAPHY_TYPE}.shp}"
...
RUN_OUTPUT_DIR="${RUN_OUTPUT_DIR:-data/outputs/${RUN_NAME}}"
